...
def read_data(sequences_file, RNAplfolds_file, annotations_file):
    ...
    with open(sequences_file) as seq_data, open(RNAplfolds_file) as RNAplfold_data, open(annotations_file) as annot_data:
        sequences_uncoded, sequences, comp_sequences, RNAplfolds, lengths, structures =list(), list(), list(), list(),list(), list()
        while True:
...
def arange_data(sequences, structures, RNAplfolds, select_x_data,L,return_x_data_as_list_or_array):
    '''
    Arange icSHAPE data for training and testing (sequence, SHAPE and RNAplfold information)
    Denote by 'l' the length of the current RNA sequence and by 'L' the windows size.
    ...
    into 'l' subsequences of length 2L+1 each. Every subsequence represents a nucleotide and its 2L adjacent neighbors (from both flanks symmetrically).
    ...
    'l' data points, each represented as a matrix of size (2L + 1) X 5. Finally, these steps are applied to every RNA sequence in the dataset.
    ...
    Args:
        ...
        return_x_data_as_list_or_array: a string indicating whether to return x_data as a python list or as numpy array.
        
    Returns:
        .
        all_x_data: Python list, each element* represents a single trancript. [number_of_transcripts X 1]
            Each element* contain a 'l'-long list of 2-d numpy arrays** (one numpy array for every nucleotide (and its 2L adjacent neighbors) in the transcript).
            Each numpy array** is a one-hot encoded subsequence concatenated by its corresponding base-pairing probabilities (a middle nucleotide and its 2L adjacent neighbors). 
            Each numpy array** is of size [(2L + 1) X (alphabet_size+1)]   
            
            if return_x_data_as_list_or_array == 'as numpy array' -> all numpy arrays** from all transcripts are concateneated together along the row axis 
            to form one numpy array of size  [total number of nucleotides in all transcripts X (alphabet_size+1)]
    ...
    all_y_data = np.concatenate(structures).ravel()
    ...
    # zero padding L bottom lines
    
    array1 = [np.append(np.asarray(x, dtype=np.float32), np.zeros((L, num_features),dtype='float32'), axis=0) for x in x_data]
    # zero padding L top lines
    array2 = [np.append(np.zeros((L, num_features),dtype='float32'), x, axis=0) for x in array1]
    # for each nucleotide, Look at the 2L neighbours (from front and  back symmetrically) and store this information along a new dimension
    array3 = [[x[i:i+2*L+1] for i in range(0,len(x)-(2*L))] for x in array2]

  
    if return_x_data_as_list_or_array == "as_array":
        #concatenate
        all_x_data = np.concatenate(array3)
    elif return_x_data_as_list_or_array == "as_list":
        all_x_data = array3
        
    
    return(all_x_data, all_y_data,num_features)
    
def write_predictions(filepath_results, neighbours, i, sequences_uncoded, y_pred_prob):
    '''
    ...
    '''
    predictions_filename = (filepath_results + 'predictions' +'_neighbours_'+str(2*neighbours)+'_epoch_'+ str(i)+'.txt');
...
def write_net_details(directive, in_vivo_vitro, seq_file, RNAplfold_file, struct_file,sequences_uncoded,
    ...
    Args:
        ...
        optimizer_:  Network parameter - optimizer
        lr_: A network parameter - learning rate
        decay_: A network parameter - decay
        filepath_results: Path to the directory which will store the output file  
        neighbours_vec_str: A string indicating the number of neighboures
    ...
    print(directive,'\n')
    print(in_vivo_vitro)
    print('Sequences file: ', seq_file)
    print('RNAplfold file: ', RNAplfold_file)
    print('Structure file: ', struct_file)
    print('Number of RNA sequences in the test set:', len(sequences_uncoded),'\n',
          'Total number of nucleotides in the test set:', sum(lengths),'\n',
          'Batch size:', batch_,'\n',
          '(add_sequences, add_RNAplfold) feature:', select_x_data, '\n', 
          'Loss function:',loss_, '\n', 
          'Activation function of last layer:',activation_last_layer,'\n',
    ...
    if directive == "predict":
        name = "performance_"
    else: #directive == "train"
        name = "net_details_"
    if not os.path.exists(filepath_results):    
        os.makedirs(filepath_results) 
    output_file = open(filepath_results + name + neighbours_vec_str + ".txt", "w") 
    output_file.write(directive)
    output_file.write('\n')
    output_file.write('Sequences file: ''{}\n'.format(seq_file))
    output_file.write('RNAplfold file: ''{}\n'.format(RNAplfold_file))
    output_file.write('Structure file: ''{}\n'.format(struct_file))
    output_file.write('Number of RNA sequences in the test set:''{}\n'.format(len(sequences_uncoded)))
    output_file.write('Total number of nucleotides in the test set:''{}\n'.format(sum(lengths)))
    output_file.write('Batch size:''{}\n'.format(batch_))
    output_file.write('(add_sequences, add_RNAplfold) feature:''{}\n'.format(select_x_data))
    output_file.write('Loss function:''{}\n'.format(loss_))
    output_file.write('Activation function of last layer:''{}\n'.format(activation_last_layer))
...
def calc_net_performance(y_pred_prob, y_true,neighbours):
    '''
...
def calc_and_write_net_performance(output_file, neighbours, model_name, i, y_pred_prob, y_true):
    '''
    Write network performance to a text file (calls to 'calc_net_performance' function to calculate the performance )

    Args:
        output_file: Path to the directory which will store the output file  
        neighbours: Number of adjacent neighbors (from both flanks symmetrically)
    ...
    if i == 1: # First epoch
        ...
        print('Number of neighbours:', 2*neighbours, '\n\n')
    pearson_corr1, mse, mae = calc_net_performance(y_pred_prob, y_true,neighbours)            
    # Print network performance
    print(model_name)
    print('Number of epochs:', i, '\n')
    print('Network architecture:')
    print("Pearson Correlation between y_pred and y_true:\n",pearson_corr1)
    print("Mean Squared Error between y_pred and y_true:\n",mse)
    print("Mean Absolute Error between y_pred and y_true:\n",mae, "\n\n")   
